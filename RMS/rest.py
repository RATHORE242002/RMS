...
def system():
    ...
    def Database():
        ...
        # creating bill table
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS Restaurantrecords(ordno text,piz text,bur text,ice text, dr text,cs text,pt text,ff text,sp text,mo text,cp text, ct text,sb text,tax text,sr text,tot text)")
    ...
    icecream = StringVar()
    drinks = StringVar()
    cheseSandwich = StringVar()
    paneerTikka = StringVar()
    ...
    def tottal():
        ...
        order =float (orderno.get())
        pi = float(pizza.get())
        bu = float(burger.get())
        ice = float(icecream.get())
        dr = float(drinks.get())
        cs = float( cheseSandwich.get())
        pt = float(paneerTikka.get())
        ff = float(frenchFries.get())
        sp = float(springRolls.get())
        mo = float(momos.get())
        cp = float(chilliPotato.get())
        ...
        costofmeal = (costpi + costbu + costice + costdr + costcs  )
        ptax = ((costpi + costbu + costice + costdr + costcs + costpt + costff + costsp + costmo + costcp ) * 0.18)
        sub = (costpi + costbu + costice + costdr + costcs + costpt + costff + costsp + costmo + costcp )
        ser = ((costpi + costbu + costice + costdr + costcs + costpt + costff + costsp + costmo + costcp ) / 99)
        paidtax = str(ptax)
        Service = str(ser)
    ...
    # Topframe
    topframe = Frame(root, bg="blue", width=1600, height=50)
    ...
    my_tree['columns'] = ("ordno", "piz", "bur", "ice", "dr", "cs","pt","ff","sp","mo","cp", "ct", "sb", "tax", "sr", "tot")
    ...
    my_tree.column("ordno", anchor=CENTER, width=80, minwidth=20)
    my_tree.column("piz", anchor=CENTER, width=60, minwidth=20)
    my_tree.column("bur", anchor=CENTER, width=50, minwidth=20)
    my_tree.column("ice", anchor=CENTER, width=80, minwidth=20)
    my_tree.column("dr", anchor=CENTER, width=50, minwidth=20)
    my_tree.column("cs", anchor=CENTER, width=100, minwidth=20)
    my_tree.column("pt", anchor=CENTER, width=100, minwidth=20)
    my_tree.column("ff", anchor=CENTER, width=100, minwidth=20)
    my_tree.column("sp", anchor=CENTER, width=100, minwidth=20)
    my_tree.column("mo", anchor=CENTER, width=80, minwidth=20)
    my_tree.column("cp", anchor=CENTER, width=100, minwidth=20)
    my_tree.column("ct", anchor=CENTER, width=50, minwidth=20)
    my_tree.column("sb", anchor=CENTER, width=100, minwidth=20)
    my_tree.column("tax", anchor=CENTER, width=50, minwidth=20)
    my_tree.column("sr", anchor=CENTER, width=100, minwidth=20)
    my_tree.column("tot", anchor=CENTER, width=50, minwidth=20)
    ...
    my_tree.heading("cs", text="CheseSandwich", anchor=CENTER)
    my_tree.heading("pt", text="PaneerTikka", anchor=CENTER)
    my_tree.heading("ff", text="FrenchFries", anchor=CENTER)
    my_tree.heading("sp", text="SpringRolls", anchor=CENTER)
    my_tree.heading("mo", text="Moms", anchor=CENTER)
    my_tree.heading("cp", text="ChilliPotato", anchor=CENTER)
    ...
    def add():
        ...
        ices = icecream.get()
        drinkss = drinks.get()
        ches = cheseSandwich.get()
        pan = paneerTikka.get()
        fre = frenchFries.get()
        spr = springRolls.get()
        mm = momos.get()
        chi = chilliPotato.get()
        costs = cost.get()
        subtotals = subtotal.get()
        taxs = tax.get()
        services = service.get()
        totals = total.get()
        if orders == "" or pizzas == "" or burgers == "" or ices == "" or drinkss == "" or ches == "" or pan == "" or fre == "" or spr == "" or mm == "" or chi == "" or costs == "" or subtotals == "" or taxs == "" or services == "" or totals == "":
            messagebox.showinfo("Warning", "Please fill the empty field!!!")
        else:
            connectn.execute(
                'INSERT INTO Restaurantrecords (ordno, piz, bur , ice ,dr,cs,pt,ff,sp,mo,cp ,ct ,sb ,tax, sr, tot) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                (orders, pizzas, burgers, ices, drinkss, ches , pan, fre, spr, mm, chi, costs, subtotals, taxs, services, totals));
            connectn.commit()
    ...
    # Top part
    main_lbl = Label(topframe, font=('Calibri', 25, 'bold'), text="Restaurant Management System", fg="Black",
                   anchor=W)
    ...
    drinktxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
     ...
     #cheseSandwich
    chlbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="CheseSandwich", fg="black", bd=5, anchor=W).grid(row=6,
                                                                                                            column=0)
    chtxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                     textvariable=cheseSandwich).grid(row=6, column=1)
    # paneerTikka
    panlbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="PaneerTikka", fg="black", bd=5, anchor=W).grid(row=7,
                                                                                                            column=0)
    pantxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                     textvariable=paneerTikka).grid(row=7, column=1)
    # frenchFries
    frelbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="FrenchFries", fg="black", bd=5, anchor=W).grid(row=8,
                                                                                                            column=0)
    fretxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                     textvariable=frenchFries).grid(row=8, column=1)
    # springRolls
    sprlbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="SpringRolls", fg="black", bd=5, anchor=W).grid(row=9,
                                                                                                            column=0)
    sprtxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                     textvariable=springRolls).grid(row=9, column=1)
    # momos
    mmlbl = Label(leftframe, font=('Calibri', 16, 'bold'), text=" Momos", fg="black", bd=5, anchor=W).grid(row=10,
                                                                                                            column=0)
    mmtxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                     textvariable= momos).grid(row=10, column=1)
    # chilliPotato
    chilbl = Label(leftframe, font=('Calibri', 16, 'bold'), text=" ChilliPotato", fg="black", bd=5, anchor=W).grid(row=11,
                                                                                                            column=0)
    chitxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                    textvariable= chilliPotato).grid(row=11, column=1)
     
    # cost
    costlbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Cost", bd=5, anchor=W).grid(row=12, column=0)
    costtxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                    textvariable=cost).grid(row=12, column=1)
    # subtotal
    sublbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Subtotal", bd=5, anchor=W).grid(row=13, column=0)
    subtxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                   textvariable=subtotal).grid(row=13, column=1)
    # tax
    taxlbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Tax", bd=5, anchor=W).grid(row=14, column=0)
    taxtxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                   textvariable=tax).grid(row=14, column=1)
    # service
    servicelbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Service", bd=5, anchor=W).grid(row=15,   column=0)
    servicetxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                       textvariable=service).grid(row=15, column=1)
    # total
    totallbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Total", bd=5, anchor=W).grid(row=16,
                                                                                                          column=0)
    totaltxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                     textvariable=total).grid(row=16, column=1)
    # ---button--
    ...
    def menu():
         ...
         lblpricep = Label(roott, font=("Calibri", 20, "bold"), text="220/-", fg="blue", bd=10)
         lblpricep.grid(row=1, column=3)
         lblburger = Label(roott, font=("Calibri", 20, "bold"), text="Burger", fg="Blue", bd=10)
         lblburger.grid(row=2, column=0)
         lblpriceb = Label(roott, font=("Calibri", 20, "bold"), text="60/-", fg="blue", bd=10)
         lblpriceb.grid(row=2, column=3)
         ...
         lblcheseSandwich = Label(roott, font=("Calibri", 20, "bold"), text="CheseSandwich", fg="Blue", bd=10)
         lblcheseSandwich.grid(row=5, column=0)
         lblpricec = Label(roott, font=("Calibri", 20, "bold"), text="90/-", fg="blue", bd=10)
         lblpricec.grid(row=5, column=3)
         lblpaneerTikka = Label(roott, font=("Calibri", 20, "bold"), text="PaneerTikka", fg="Blue", bd=10)
         lblpaneerTikka.grid(row=6, column=0)
         lblpricec = Label(roott, font=("Calibri", 20, "bold"), text="100/-", fg="blue", bd=10)
         lblpricec.grid(row=6, column=3)
         lblfrenchFries = Label(roott, font=("Calibri", 20, "bold"), text="FrenchFries", fg="Blue", bd=10)
         lblfrenchFries.grid(row=7, column=0)
         lblpricef = Label(roott, font=("Calibri", 20, "bold"), text="50/-", fg="blue", bd=10)
         lblpricef.grid(row=7, column=3)
         lblspringRolls = Label(roott, font=("Calibri", 20, "bold"), text="SpringRolls", fg="Blue", bd=10)
         lblspringRolls.grid(row=8, column=0)
         lblprices = Label(roott, font=("Calibri", 20, "bold"), text="80/-", fg="blue", bd=10)
         lblprices.grid(row=8, column=3)
         lblmomos = Label(roott, font=("Calibri", 20, "bold"), text="Momos", fg="Blue", bd=10)
         lblmomos.grid(row=9, column=0)
         lblpricem = Label(roott, font=("Calibri", 20, "bold"), text="40/-", fg="blue", bd=10)
         lblpricem.grid(row=9, column=3)
         lblchilliPotato = Label(roott, font=("Calibri", 20, "bold"), text="ChilliPotato", fg="Blue", bd=10)
         lblchilliPotato.grid(row=10, column=0)
         lblpricec = Label(roott, font=("Calibri", 20, "bold"), text="80/-", fg="blue", bd=10)
         lblpricec.grid(row=10, column=3)
         roott.mainloop()
